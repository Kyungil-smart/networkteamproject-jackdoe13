...
## 2-3 각 함수 별 개념
- Awake
1. Start() 전에 호출되며 오브젝트 마다 1번만 호출
2. 오브젝트 생성 직후 실행
3. 오브젝트 비활성화 상태일 때 호출 안됨
4. 오브젝트 활성화 / script 비활성화 상태에 호출 가능.
5. 자기 자신 초기화
6. Rigidbody 가져오기
7. Animator 가져오기
8. AudioSource 가져오기
...
2. 첫 번째 프레임 업데이트 전에 호출
3. Script 인스턴스가 활성화 된 경우에만 호출
4. 오브젝트가 게임플레이 도중 인스턴스화 될 떄 실행 안됨.
5. 오브젝트 활성화 / Script 비활성화 상태에선 호출 불가능.
6. UI 초기 값 반영
7. GameManager 데이터 가져오기
8. 플레이어 위치 참조
9. 시작 연출 실행
...
** Awake는 모든 오브젝트가 초기화되고 호출되고, Start는 스크립트 인스턴스가 활성화된 경우에만 호출.

** Awake는 모든 오브젝트가 초기화되고 호출되기 때문에 다른 스크립트와 연결을 보장할 수 있다.

** 다른 스크립트들을 많이 참조하는 프로젝트일수록 Awake()의 사용을 권장.

** Awake()를 사용하지 않고 Start()로 초기화와 인스턴스 활성후 호출을 둘다 사용하면 자신이 인스턴스화한 유닛의 스크립트들이 start로 초기화된 정보가 없기 때문에 nullreference오류를 발생.

## Start()와 Init() 차이와 특징

1. Start()
    ```
    a. 게임오브젝트가 활성화 될때 Awake()이후에 한번 실행되는 함수.
    b. Init() 대신 초기화 함수를 사용하면
    ```

2. Init()
    ```
    a. 클래스가 MonoBehaviour를 상속받을 때 기본적으로 생성자를 사용하지 못하기 때문에 직접 생성자의 역할을 하는 메서드를 구현해준 것.
    b. 원하는 시점에 직접 초기화 타이밍을 제어하고 외부에서 필요한 데이터(파라미터)를 안전하게 전달하기 위해 사용.
    ```

3. 순서
    a. Init()이 먼저 실행되고 그 다음 Start()가 실행된다.
...
- Init()
    1. https://zizh.tistory.com/78
    2. https://qua28.tistory.com/152
    3. https://everenew.tistory.com/238